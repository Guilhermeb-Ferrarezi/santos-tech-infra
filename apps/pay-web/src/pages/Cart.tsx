    ...
    (async () => {
      try {
        ...
        if (slug && !cart.some((l) => l.product.slug === slug)) {
          // Acesso direto a /<slug>: se o produto for recorrente, ele não entra no
          // carrinho multi-item — manda pro checkout de assinatura.
          const prod = await api.product(slug).catch(() => null);
          if (prod?.recurring) {
            if (!cancelled) nav(`/assinar/${slug}`, { replace: true });
            return;
          }
