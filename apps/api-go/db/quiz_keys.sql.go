...
func (q *Queries) GetQuizAccessKeyByHash(ctx context.Context, keyHash string) (GetQuizAccessKeyByHashRow, error) {
	row := q.db.QueryRow(ctx, getQuizAccessKeyByHash, keyHash)
	var i GetQuizAccessKeyByHashRow
	err := row.Scan(
		&i.ID,
		&i.Label,
		&i.DailyLimit,
...
const reserveQuizKeyUsage = `-- name: ReserveQuizKeyUsage :one
...
// quiz_keys.go.
func (q *Queries) ReserveQuizKeyUsage(ctx context.Context, id int64) (int64, error) {
	row := q.db.QueryRow(ctx, reserveQuizKeyUsage, id)
	var id_2 int64
	err := row.Scan(&id_2)
	return id_2, err
}
