...
func (s *Server) registerPortalRoutes(mux *http.ServeMux) {
	...
	// Gerador de currículo, fase 2: botão "Melhorar com IA" — self-service,
	...
	mux.HandleFunc("GET /portal/turmas-abertas", s.portalRead("portal_turmas", s.handleTurmasAbertas))
	...
	mux.HandleFunc("PATCH /portal/classes/{classId}/students/{studentId}", s.rateLimit(30, min, s.portalWrite("portal_turmas", s.handlePortalSetStudentIndividual)))
	mux.HandleFunc("GET /portal/classes/{classId}/teachers", s.portalRead("portal_turmas", s.handlePortalListClassTeachers))
	mux.HandleFunc("POST /portal/classes/{classId}/teachers", s.rateLimit(20, min, s.portalWrite("portal_turmas", s.handlePortalAddClassTeacher)))
	mux.HandleFunc("DELETE /portal/classes/{classId}/teachers/{teacherId}", s.adminGuard(s.handlePortalRemoveClassTeacher))
